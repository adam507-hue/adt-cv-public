...
def find_most_congested_checkpoint(
    ...
    for key in data:
        for prex in allowed_queues:
            if(prex in key):
                keys[prex].append(key)

    histogram(data)
    for key, datKey in keys.items():
        histogram(data)
        print("------------------")
        for i in range(1,25):
            size = 0
            ...
            #filterData1 = filter_data_time(data, time)
            #filterData2 = filter_data_time(data, int(time-3600))

            #size = get_q_size(filterData1, datKey) - get_q_size(filterData2, datKey)

            """
            for dat in datKey:
                size += get_check_size_time(data, dat, time - 3600, time)
                #print(f"{int((time)/3600)}:00 - {size} que: {dat}")
                if(size > best_size):
                    best_size = size
                    best_time = time
                    best_ckpt = key
...
def get_check_size_time(data, check, time1, time2) -> int:
    ret = set()

    for rec in data[check]:
        if(time1 <= rec.time and rec.time <= time2):
            ret.add(rec)
